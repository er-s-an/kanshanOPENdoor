...
> **归档批注（2026-09-14）**：本文是 9/13 的三路调研整合，已归档。吸收情况：
> - **已吸收**：A1 稀有度（结局评级徽章）、B2 正向表述（改写式提示词）、B3 关键词触发（合并版 dialogue topics 的 keywords 匹配）、C1-C6 管线 QA 体系（validate 门控校验/重试/仿真）、Façade「确定性主干+受限即兴」理论背书（正是现行架构）
> **被新设计取代**：A3 目标进度可视化（改为话题「已聊过」标记）、B1 硬规则后置（改写式提示词用单条 user 消息承载，不再需要）
> **仍有效待用**：A5 复盘「你离真相只差一步」、B5 NPC 情绪 sticky、ChatHaruhi 原文台词检索（zhida 换岗后的保声口方案）、档二全部理论背书（计划书弹药）
> **档三「不做清单」依然全部成立**（语音/UGC 市场/亲密度付费/全 AI 生成/多人剧本杀/自由侦探/诱导分享——一个坑都没踩）。
> 以下原文保留，作为计划书理论引用与后续迭代候选库。
