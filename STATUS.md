...
> **历史版本：完整首章重做 v2.2（2026-09-13）**。此前两段 playable 小样例退出默认入口。新版恢复三篇官方多幕故事；蓝血 27 节点、近视眼 24 节点，主线聊天／搜证／查证／行动／结算已接通。以 [PRD-v2.2](PRD-v2.2.md) 和 [本次修复验收](docs/REBUILD-ACCEPTANCE-20260913.md) 为最新事实，下面旧记录按历史阅读。


> 2026-09-13 更新：产品方向以 [PRD-v2.1.md](PRD-v2.1.md) 的「玩法库 × 前人留痕」为准；已验证实现及后续交接见 [开发记录](docs/DEVELOPMENT-20260913.md)。下文保留凌晨的 V1 开发历史，不代表 V2 已全部实现。
...
- 凌晨 quota 历史快照（不能作为当前剩余额度）：**直答 5000**、搜索 5000×2、热榜 100、问答 100、用户数据 10000、创作 100、知识库 500、小工具 10；新版 Skill 说明 quota 返回当前自然日额度；以实时查询为准
...
5. 产品说明计划书（必交，初审重点）——PRD.md 是内核，强化社区契合度表述
6. 演示视频（选交加分）
